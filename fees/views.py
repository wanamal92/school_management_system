...
def list_fee_payments(request):
    payments = FeePayment.objects.all().order_by('student__full_name')

    # Pagination setup
    paginator = Paginator(payments, 10)  # Show 10 students per page
    page_number = request.GET.get('page')
    payments_page = paginator.get_page(page_number)
    return render(request, 'fees/list_fee_payments.html', {'payments': payments_page})
...
def list_fee_types(request):
    ...
    # Pagination setup
    paginator = Paginator(fee_types, 10)  # Show 10 students per page
    page_number = request.GET.get('page')
    fee_typess_page = paginator.get_page(page_number)

    return render(request, 'fees/list_fee_types.html', {'fee_types': fee_typess_page})
...
def generate_invoice_pdf(request, pk):
    ...
    p.line(50, table_start_y - 25, width - 50,
           table_start_y - 25)  # Header bottom
    ...
    p.setFont("Helvetica-Bold", 12)
    p.setFillColor(dark_color)
    p.drawString(width - 250, payment_info_y - 20,
                 "Sri Ganalaankra Maha Pirivena")
    p.setFont("Helvetica", 10)
    p.setFillColor(colors.black)
    ...
    if payment.balance > 0:
        ...
        p.drawString(50, payment_info_y - 90,
                     f"Outstanding Balance: Rs {payment.balance}")

    # Footer
    p.setFont("Helvetica", 8)
    p.setFillColor(colors.gray)
    p.drawString(
        50, 50, "For any queries, please contact us at: nenalakara1985@gmail.com")
    p.drawString(
        50, 35, f"Generated on: {datetime.now().strftime('%d %B %Y at %H:%M')}")
